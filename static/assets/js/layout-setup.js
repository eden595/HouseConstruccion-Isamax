...
menuLinks.forEach((link) => {
  const linkHref = link.getAttribute("href"); // Get the href attribute of the link
    ...
    if (currentUrl.includes(linkHref)) {
        // link.classList.add("active"); // REMOVED: Rely on server-side active class to avoid substring matching issues
    ...
    // Open all parent dropdowns for the active link
        openParentDropdowns(link); // Only open parents if the link is actually active
      }
    }
